...
- 初次验收时 Step-5 通过 `anthropic-messages` 请求返回 HTTP 403；没有静默换模型。已生成计划和首区块在恢复失败后仍保留。不能据此断言是额度/账号/服务端策略的哪一种原因。随后复测的通道、工具任务与工坊分别记录于 [补充验收](runtime-followup-2026-09-25.md)，不以通道恢复替代长任务验收。
