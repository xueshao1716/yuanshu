...
## 2026-09-24 用户授权后的发布收尾

用户明确授权在保留内容质量与媒体尺寸限制的前提下重启、打包和双推。主仓继承源码逐文件比对后单独提交，再选择集成本轮修复，未直接推送现场快照分支。版本统一为 2.115.19，服务重启后健康与版本核验通过；Windows 和 Android 安装包已完成构建及内部版本核对，Android 签名与对齐验证通过。

新增实际执行引擎身份注入后，最终隔离全量测试 2026/2026、50 suites，浏览器两尺寸回归 2/2；集成后身份/版本契约 15/15，引擎目录 10/10。默认主驾仍为 pi，未隐式切换。发布产物同步到主仓提交，临时验证材料保持本地。上文天团内容质量、媒体精确像素和 Tauri 真机测试限制仍有效。

包路径、摘要、签名和验证范围见 [2.115.19 打包记录](../../packaging-2.115.19-20260924.md)。远端推送结果以发布后实际远端提交核对为准。
