...
- 代码与前端快照提交 `f3959ecedd168a03dad5307d7605af62b799fa59` 已双推，GitHub、Gitee 的 main 均逐一核对一致。
- GitHub 首次干净环境运行 `36118882691`：依赖安装、类型检查、构建通过；测试 2148 项，2142 通过、2 失败、4 按既有条件跳过。原始日志归档在证据目录的 `ci-36118882691/`，不将首次失败改写为通过。
- 命名测试把带 +08:00 的绝对时刻与固定本地时间字符串比较；UTC 环境已复现失败。修正为本地日历时间夹具，产品的本地日期命名规则不变。
- 重启包装器测试在远端超过 25 秒。测试输出改用文件，避免后台后代继承管道影响 EOF 等待；冷启动预算改为 90 秒，后代存活 180 秒。仍断言退出码 7 且返回时后代存活，没有跳过测试或修改生产重启逻辑。本机定向回归 13/13，远端复验结果以本次后续运行及交付说明为准。
- 远端 4 个跳过项来自无本机 Pi mutation queue、无 ffmpeg；不是跨环境全部能力已验收。测试修正不改变生产前端或安装包，无需重新打包。
