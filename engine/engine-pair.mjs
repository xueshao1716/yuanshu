...
export const ENGINE_CATALOG = {
  yuanshu: {
    ...
    can: ["OpenAI 兼容与 Anthropic Messages 通道（以通道配置和实测能力为准）", "工具调度：只读可并行、写互斥", "上下文命名区段 + 沙箱阶梯 + 压缩留底", "prompt 区段接缝（插件贡献，一轮只收一次）", "磁盘工作记忆 task_plan/findings/progress", "后台任务与页面连接分离，重连后恢复已记录进度", "执行检查点与副作用记录，支持受控接续", "本地规则评测与分项得分（不代表真实模型内容验收）", "VAD 情绪自己开轮注入、收轮推 SSE，不靠外置适配器"],
