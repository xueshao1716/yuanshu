...
function walk(root, out, depth = 0) {
  ...
  for (const e of entries) {
    ...
    if (/\.bak(-|$)/.test(e.name) || /\.bak$/.test(e.name)) {
      ...
      out.push({ kind: "backup", backup: p, target: p.replace(/\.bak.*$/, ""), at: st ? new Date(st.mtimeMs).toISOString() : null, size: st ? st.size : 0 });
    }
  }
}

export function createHistoryApi({ wsRoot, json, readBody }) {
  const audit = (line) => { try { fs.appendFileSync(path.join(wsRoot, "记忆", "授权记录.jsonl"), JSON.stringify(line) + "\n", "utf8"); } catch {} };
  ...
  return {
    ...
    async list(res) {
      ...
      return json(res, 200, {
        ok: true,
        backups: backups.slice(0, MAX_ITEMS).map((b) => ({ ...b, backup: b.backup.replace(wsRoot + "\\", ""), target: b.target.replace(wsRoot + "\\", "") })),
    ...
    async rollback(res, body) {
      const rel = String(body?.backup || "").replace(/^[\\/]+/, "");
      const backup = path.join(wsRoot, rel);
      const target = backup.replace(/\.bak.*$/, "");
      ...
      try {
        if (fs.existsSync(target)) fs.writeFileSync(target + ".bak-rollback-" + Date.now().toString(36), fs.readFileSync(target));
        fs.mkdirSync(path.dirname(target), { recursive: true });
        fs.writeFileSync(target, fs.readFileSync(backup));
      } catch (e) { return json(res, 500, { error: String((e && e.message) || e).slice(0, 140) }); }
      audit({ at: new Date().toISOString(), kind: "history-rollback", backup: rel, target: target.replace(wsRoot + "\\", "") });
      return json(res, 200, { ok: true, rolledBack: target.replace(wsRoot + "\\", ""), from: rel, note: "回滚前的内容已另存为 .bak-rollback-<时间>" });
    },
  };
}
