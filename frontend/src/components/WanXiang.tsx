  ...
  <div className="flex flex-wrap gap-1.5">
    {items.map(([k, label]) => (
      <button key={k} onClick={() => onChange(k)}
        className={`px-2.5 py-1 rounded-full text-[11px] chip border transition-colors duration-fast ${value === k ? 'chip-pill is-on font-medium' : 'chip-pill is-off'}`}>
        {label}
      </button>
    ))}
  </div>
          ...
          <div className="flex flex-wrap gap-1.5 mt-1.5">
            {LIGHTS.map((l) => (
              <button key={l} type="button" onClick={() => setLighting(l)}
                className={`px-2 py-1 rounded-full text-[11px] chip border ${lighting === l ? 'chip-pill is-on' : 'chip-pill is-off'}`}>{l}</button>
            ))}
          </div>
        </Field>
        <Field label="氛围">
          ...
          <div className="flex flex-wrap gap-1.5 mt-1.5">
            {MOODS.map((m) => (
              <button key={m} type="button" onClick={() => setMood(m)}
                className={`px-2 py-1 rounded-full text-[11px] chip border ${mood === m ? 'chip-pill is-on' : 'chip-pill is-off'}`}>{m}</button>
            ))}
          </div>
        </Field>
        <Field label="风格">
          ...
          <div className="flex flex-wrap gap-1.5 mt-1.5">
            {STYLES.map((st) => (
              <button key={st} type="button" onClick={() => setStyle(st)}
                className={`px-2 py-1 rounded-full text-[11px] chip border ${style === st ? 'chip-pill is-on' : 'chip-pill is-off'}`}>{st}</button>
            ))}
          </div>
        </Field>
        ...
        <Field label="输出平台">
          <div className="flex gap-1.5">
            {[['dreamina', '即梦'], ['mj', 'MJ'], ['sd', 'SD']].map(([k, l]) => (
              <button key={k} onClick={() => setPlatform(k)} className={`px-2.5 py-1 rounded-full text-[11px] chip border transition-colors duration-fast ${platform === k ? 'chip-pill is-on' : 'chip-pill is-off'}`}>{l}</button>
            ))}
          </div>
        </Field>
