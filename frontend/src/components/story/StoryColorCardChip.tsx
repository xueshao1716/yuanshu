import { resolveColorCard, colorCardGradient } from '../../theme/colorcards.mjs'
...
// 混在一起显示会让人以为整片都改了。
export default function StoryColorCardChip({ colorCardId, toneOverride = '', note = '' }: { colorCardId?: string; toneOverride?: string; note?: string }) {
  const card = resolveColorCard(colorCardId) as { id: string; name: string; top: string; bottom: string } | null
  if (!card) return null
