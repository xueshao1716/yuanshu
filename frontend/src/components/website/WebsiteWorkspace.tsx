...
export default function WebsiteWorkspace({initial,onBack}:{initial:WebsiteProject;onBack:()=>void}) {
  ...
  const [instruction,setInstruction]=useState(''),[tab,setTab]=useState('edit'),[device,setDevice]=useState('desktop'),[full,setFull]=useState(false)
  ...
  async function generate(local:boolean){await guard(async()=>{
    ...
    const run=await websites.generate(project.id,{model:model.value,baseVersion,targetId,instruction})
